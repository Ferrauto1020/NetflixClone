...
namespace NetflixClone.Services
{
    ...
    {
        private const string ApiKey = "";//insert your API
        public const string TmdbHttpClientName = "TmdbClientName";
        ...
        {
            var genreWrapper = await HttpClient.GetFromJsonAsync<GenreWrapper>($"{TmdbUrls.MovieGenres}&api_key={ApiKey}");
            return genreWrapper.Genres;
        }
        ...
        public async Task<IEnumerable<Media>> GetNetflixOriginalsAsync() => await GetMediasAsync(TmdbUrls.NeflixOriginals);
        ...
        {
            var details = await HttpClient.GetFromJsonAsync<MovieDetail>($"{TmdbUrls.GetMovieDetails(id, type)}&api_key={ApiKey}");
            return details;
        }
        ...
        private async Task<IEnumerable<Media>> GetMediasAsync(string url)
        {
            ...
            Console.WriteLine($"Requesting URL: {fullUrl}"); // Log per vedere l'URL completo

            // Aggiungi un controllo dell'errore HTTP
            var response = await HttpClient.GetAsync(fullUrl);
            if (!response.IsSuccessStatusCode)
            {
                Console.WriteLine($"Error: {response.StatusCode}");
                // Potresti lanciare un'eccezione o gestire l'errore in qualche altro modo
                return Enumerable.Empty<Media>();
            }
            var imgUrlBase = "https://image.tmdb.org/t/p/original";
            var trendingMoviesCollection = await HttpClient.GetFromJsonAsync<Movie>($"{url}&api_key={ApiKey}");
            return trendingMoviesCollection.results.Select(
                 r => new Media
                 {
                     Id = r.id,
                     DisplayTitle = r.title ?? "No Title",
                     Overview = r.overview,
                     ReleaseDate = r.release_date,
                     Thumbnail = $"{imgUrlBase}{r.poster_path}",
                     ThumbnailSmall = $"{imgUrlBase}{r.ThumbnailSmall}"
                 });
        }
        ...
        public static class TmdbUrls
        {
            ...
            public const string NeflixOriginals = "3/discover/tv?language=en-US&with_networks=213";
